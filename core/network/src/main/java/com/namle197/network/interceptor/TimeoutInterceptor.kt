package com.namle197.network.interceptor
...
internal class TimeoutInterceptor : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        return chain
            .withConnectTimeout(
                if (connectTimeoutRequest.isNullOrEmpty()) {
                    DEFAULT_CONNECT_TIMEOUT_MS
                } else {
                    Integer.valueOf(
                        connectTimeoutRequest
                    )
                },
                TimeUnit.MILLISECONDS
            )
            .withReadTimeout(
                if (readTimeoutRequest.isNullOrEmpty()) {
            ...
            .withWriteTimeout(
                if (writeTimeoutRequest.isNullOrEmpty()) {
